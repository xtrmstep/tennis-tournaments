...
- `backend/api/admin.py` — `/api/admin` blueprint (admin-only): list users, patch active status, delete user.
...
| `/admin/users` | `src/views/AdminUsersView.vue` | Website Admin only | Table of all registered users with profile/active status and last-updated time. Clicking a row opens a sidebar showing events (placeholder). Admins can enable/disable users and delete them. |
